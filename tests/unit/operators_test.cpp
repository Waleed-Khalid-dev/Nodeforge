...
TEST(OperatorsTest, ConstantChanOpOutputs) {
    ...
    EXPECT_EQ(buf.names[0], "tx");
    EXPECT_EQ(buf.names[1], "ty");
    EXPECT_EQ(buf.names[2], "tz");

    ASSERT_EQ(buf.GetSampleCount(), 10u);
    EXPECT_FLOAT_EQ(buf.data[0][0], 1.5f);
    EXPECT_FLOAT_EQ(buf.data[1][0], 2.5f);
    EXPECT_FLOAT_EQ(buf.data[2][0], 3.5f);
}
...
TEST(OperatorsTest, MathChanOpMathCalculations) {
    ...
    mathOp.GetInputPin("input")->AddConnection(constOp.GetOutputPin("output"));
    ...
    EXPECT_FLOAT_EQ(mathOp.GetOutputPin("output")->GetValue().Get<nf::ChannelBuffer>().data[0][0], 12.0f);
    ...
    EXPECT_TRUE(mathOp.Cook(ctx));
    EXPECT_FLOAT_EQ(mathOp.GetOutputPin("output")->GetValue().Get<nf::ChannelBuffer>().data[0][0], 30.0f);
}
