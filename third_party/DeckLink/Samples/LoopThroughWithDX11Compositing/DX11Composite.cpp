...
bool DX11Composite::InitDeckLink()
{
...

	{
		...
		CComPtr<IDeckLinkVideoBufferAllocatorProvider> captureAllocator(new (std::nothrow) InputAllocatorPool(mpD3DDevice));
	...
	mPlayoutAllocator = new PinnedMemoryAllocator(mpD3DDevice, VideoFrameTransfer::GPUtoCPU, 1, outputFrameRowBytes * mFrameHeight);
	...
	for (int i = 0; i < 10; i++)
	{
		...
		if (mDLOutput->CreateVideoFrameWithBuffer(mFrameWidth, mFrameHeight, outputFrameRowBytes, bmdFormat8BitBGRA, bmdFrameFlagDefault, outputFrameBuffer, &outputFrame) != S_OK)
			goto error;
...
void DX11Composite::VideoFrameArrived(IDeckLinkVideoInputFrame* inputFrame, bool hasNoInputSource)
{
	...
	if (mFastTransferExtensionAvailable)
	{
		...
		if (!allocator || !allocator->transferFrame(videoPixels, (void *)mpCaptureTexture))
			OutputDebugStringA("Capture: transferFrame() failed\n");

		allocator->waitSyncComplete(videoPixels);
	}
...
void DX11Composite::PlayoutFrameCompleted(IDeckLinkVideoFrame* completedFrame, BMDOutputFrameCompletionResult completionResult)
{
	...
	if (mFastTransferExtensionAvailable)
	{
		...
		VideoFrameTransfer::waitAPI(VideoFrameTransfer::CPUtoGPU);
	}
	...
	{
		SAFE_RELEASE(outputVideoFrameBuffer);
		LeaveCriticalSection(&pMutex);
		return;
	}
	...
	if (mFastTransferExtensionAvailable)
	{
		...
		VideoFrameTransfer::endAPI(VideoFrameTransfer::CPUtoGPU);
...
PinnedMemoryAllocator::PinnedMemoryAllocator(ID3D11Device* pD3DDevice, VideoFrameTransfer::Direction direction, unsigned cacheSize, unsigned int bufferSize) :
...
bool PinnedMemoryAllocator::transferFrame(void* address, void* gpuTexture)
{
	if (mFrameTransfer.count(address) == 0)
	{
		...
		mFrameTransfer[address] = new VideoFrameTransfer(mpD3DDevice, mBufferSize, address, mDirection);
	}
...
InputAllocatorPool::InputAllocatorPool(ID3D11Device* pD3DDevice) :
...
HRESULT InputAllocatorPool::GetVideoBufferAllocator(unsigned int bufferSize, unsigned int, unsigned int, unsigned int,
													BMDPixelFormat, IDeckLinkVideoBufferAllocator **allocator)
{
	...
	newAllocator.Attach(new (std::nothrow) PinnedMemoryAllocator(mD3DDevice, VideoFrameTransfer::CPUtoGPU, 3, bufferSize));
	if (!newAllocator)
		return E_OUTOFMEMORY;
