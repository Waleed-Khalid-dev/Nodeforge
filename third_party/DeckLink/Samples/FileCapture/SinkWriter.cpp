...
static UINT32 getDefaultStride(BMDPixelFormat pixelFormat, UINT32 frameWidth)
{
	if (pixelFormat == bmdFormat8BitYUV)
	{
		return (frameWidth * 16) / 8;
	}
	else if (pixelFormat == bmdFormat8BitBGRA)
	{
		return (frameWidth * 32) / 8;
	}
	else
	{
		return 0;
...
void SinkWriter::SinkWriterThread(CComPtr<VideoSampleQueue> videoQueue, CComPtr<AudioSampleQueue> audioQueue)
{
...
void SinkWriter::VideoSinkWriterThread(CComPtr<VideoSampleQueue> videoQueue)
{
...
HRESULT SinkWriter::ConfigureVideoEncoder(CComPtr<IDeckLinkDisplayMode> inputDisplayMode, BMDPixelFormat inputPixelFormat)
{
	...
	hr = mediaTypeIn->SetUINT32(MF_MT_DEFAULT_STRIDE, getDefaultStride(inputPixelFormat, inputDisplayMode->GetWidth()));
	if (hr != S_OK)
		goto bail;
