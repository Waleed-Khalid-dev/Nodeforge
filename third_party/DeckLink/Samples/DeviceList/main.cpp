...
static const std::list<std::pair<BMDPixelFormat, std::string>> gPixelFormats =
{
	...
	{ bmdFormat10BitYUVA,    "10-bit YUVA" },
...
static const std::list<std::pair<BMDVideoConnection, std::string>> gConnections =
{
	...
	{ bmdVideoConnectionOpticalEthernet,  "Optical Ethernet" },
	{ bmdVideoConnectionInternal,	  "Internal" },
};
...
static const std::map<BMDDuplexMode, const char*> gDuplexModes =
{
...
bool	print_output_mode(IDeckLinkOutput* deckLinkOutput, BMDVideoConnection connection, BMDVideoOutputConversionMode conversion, BMDSupportedVideoModeFlags flags, IDeckLinkDisplayMode *displayMode);
void	print_output_modes_for_setup (IDeckLinkOutput* deckLinkOutput, BMDVideoConnection connection, BMDVideoOutputConversionMode conversion, BMDSupportedVideoModeFlags flags);
void	print_output_modes (IDeckLink* deckLink, uint32_t printFlags);
bool	print_input_mode(IDeckLinkInput* deckLinkInput, BMDVideoConnection connection, BMDVideoInputConversionMode conversion, BMDSupportedVideoModeFlags flags, IDeckLinkDisplayMode* displayMode, const char* nameSuffix);
void	print_input_modes_for_setup (IDeckLinkInput* deckLinkInput, BMDVideoConnection connection, BMDVideoInputConversionMode conversion, BMDSupportedVideoModeFlags flags, const char* nameSuffix);
...
int		main (int argc, char** argv)
{
	...
	result = CoCreateInstance(CLSID_CDeckLinkAPIInformation, NULL, CLSCTX_INPROC_SERVER, IID_IDeckLinkAPIInformation, (void**)&deckLinkAPIInformation);
	if (result == S_OK)
	{
...
void	parse_arguments(int argc, char** argv, uint32_t& printFlags)
{
	...
	{
		if (strcmp(argv[i], "--anyconnections") == 0 || strcmp(argv[i], "-a") == 0)
			printFlags &= ~kPrintDisplayModeConnections;
		...
		else if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0)
		{
			printf("%s [options]\n"
			       ...
			       "    -a, --anyconnections    Display the supported modes for any connection type\n"
...
void	print_attributes (IDeckLink* deckLink, bool showConnectorAttributes)
{
	...
	dlbool_t							supported;
	int64_t								value;
	dlstring_t							stringValue;
	HRESULT								result;
	int64_t								deckLinkEthernetConnectorCount;
	...
	if (result == S_OK)
	{
		printf(" %-40s %s\n", "Duplex Mode:", gDuplexModes.at((BMDDuplexMode)value));
	}
	else
	{
	...
	result = deckLinkAttributes->GetFlag(BMDDeckLinkHasMonitorOut, &supported);
	if (result == S_OK)
	{
		printf(" %-40s %s\n", "Has Monitor Out ?", supported ? "Yes" : "No");
	}
	else
	{
		fprintf(stderr, "Could not query the Has Monitor Out attribute- result = %08x\n", result);
	}

	result = deckLinkAttributes->GetInt(BMDDeckLinkNumberOfEthernetConnectors, &deckLinkEthernetConnectorCount);
	if (result == S_OK)
	{
		printf(" %-40s %lld\n", "Number of Ethernet connectors:", deckLinkEthernetConnectorCount);
	}
	else
	{
		if (result != E_NOTIMPL)
			fprintf(stderr, "Could not query BMDDeckLinkNumberOfEthernetConnectors. result = %08x\n", result);
		deckLinkEthernetConnectorCount = 0;
	}

	for (auto i = 0; i < deckLinkEthernetConnectorCount; i++)
	{
		result = deckLinkAttributes->GetStringWithParam(BMDDeckLinkParamEthernetMACAddress, i, &stringValue);
		if (result == E_NOTIMPL)
			continue;

		if (result == S_OK)
		{
			auto stdString = DlToStdString(stringValue);
			DeleteString(stringValue);

			auto heading = "Port " + std::to_string(i) + " MAC Address:";
			printf(" %-40s %s\n", heading.c_str(), stdString.c_str());
		}
		else
		{
			fprintf(stderr, "Could not query BMDDeckLinkParamEthernetMACAddress. result = %08x\n", result);
		}
	}
	
	result = deckLinkAttributes->GetInt(BMDDeckLinkMezzanineType, &value);
	if (result == S_OK || result == E_NOTIMPL)
	{
		printf(" %-40s %s\n", "Mezzanine supported ?", result == E_NOTIMPL ? "No" : "Yes");
		if (result == S_OK)
		{
			printf(" %-40s ", "Mezzanine type:");
			switch (value) {
			case bmdMezzanineTypeNone:
				printf("None\n");
				break;
			case bmdMezzanineTypeHDMI14OpticalSDI:
				printf("HDMI 1.4 Optical SDI\n");
				break;
			case bmdMezzanineTypeQuadSDI:
				printf("Quad SDI\n");
				break;
			case bmdMezzanineTypeHDMI20OpticalSDI:
				printf("HDMI 2.0 Optical SDI\n");
				break;
			case bmdMezzanineTypeHDMI21RS422:
				printf("HDMI 2.1 RS422\n");
				break;
			default:
				printf("Unexpected\n");
				break;
		...
		fprintf(stderr, "Could not get mezzanine type. result = %08x\n", result);
	...
	for (auto pixelFormat: gPixelFormats)
	{
		...
		if (deckLinkOutput->DoesSupportVideoMode(connection, requestedMode, pixelFormat.first, conversion, flags, &retMode, &supported) == S_OK && supported)
		{
			...
			if (! printedMode)
			{
				...
				bool requestedPsF = (flags & bmdSupportedVideoModePsF) != 0;
				bool converted = actualMode != requestedMode;
				modeWidth = (int)displayMode->GetWidth();
				modeHeight = (int)displayMode->GetHeight();
...
void	print_output_modes_for_setup (IDeckLinkOutput* deckLinkOutput, BMDVideoConnection connection, BMDVideoOutputConversionMode conversion, BMDSupportedVideoModeFlags flags)
{
	...
	{
		printedSection = print_output_mode(deckLinkOutput, connection, conversion, flags, displayMode) || printedSection;
		displayMode->Release();
	}
...
void	print_output_modes (IDeckLink* deckLink, uint32_t printFlags)
{
				...
				if (connection.first == bmdVideoConnectionSDI || connection.first == bmdVideoConnectionOpticalSDI)
				{
					for (std::list<std::pair<BMDSupportedVideoModeFlags, std::string>>::const_reverse_iterator links = gSDILinks.rbegin(); links != gSDILinks.rend(); links++)
					{
						...
						if (conversion.first == bmdNoVideoOutputConversion)
							...
							printf("%s %s %s output:\n\n", connection.second.c_str(), links->second.c_str(), conversion.second.c_str());

						print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, links->first);
						print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(links->first | bmdSupportedVideoModePsF));
						if (multilinkSupported)
						{
							print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(links->first | bmdSupportedVideoModeDualStream3D));
							print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(links->first | bmdSupportedVideoModePsF | bmdSupportedVideoModeDualStream3D));
						}
					}
				}
				else
				{
					if (connection.first != bmdVideoConnectionUnspecified)
					{
						if (conversion.first == bmdNoVideoOutputConversion)
							printf("%s output:\n\n", connection.second.c_str());
						else
							printf("%s %s output:\n\n", connection.second.c_str(), conversion.second.c_str());
					}
					else if (conversion.first != bmdNoVideoOutputConversion)
					{
						printf("%s output:\n\n", conversion.second.c_str());
					}

					print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, bmdSupportedVideoModeDefault);
					print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, bmdSupportedVideoModePsF);
					print_output_modes_for_setup(deckLinkOutput, connection.first, conversion.first, bmdSupportedVideoModeDualStream3D);
				}
			}
		}
	}

	// List all supported keyer display modes
	printf("Supported keyer display modes:\n\n");
	...
	for (const auto& connection: gConnections)
	{
		if (connection.first == bmdVideoConnectionUnspecified || (ports & connection.first))
		{
			...
			if (keyingSupported && (connection.first == bmdVideoConnectionUnspecified ||
				connection.first == bmdVideoConnectionSDI || connection.first == bmdVideoConnectionOpticalSDI ||
				connection.first == bmdVideoConnectionEthernet || connection.first == bmdVideoConnectionOpticalEthernet ||
				connection.first == bmdVideoConnectionInternal))
			{
				if (connection.first != bmdVideoConnectionUnspecified)
				{
					printf("%s keyer output:\n\n", connection.second.c_str());
				}
				print_output_modes_for_setup(deckLinkOutput, connection.first, bmdNoVideoOutputConversion, bmdSupportedVideoModeKeying);
			}
		 }
	}
...
void	print_input_modes_for_setup (IDeckLinkInput* deckLinkInput, BMDVideoConnection connection, BMDVideoInputConversionMode conversion, BMDSupportedVideoModeFlags flags, const char* nameSuffix)
{
	...
	while (displayModeIterator->Next(&displayMode) == S_OK)
	{
		printedSection = print_input_mode(deckLinkInput, connection, conversion, flags, displayMode, nameSuffix) || printedSection;
		displayMode->Release();
	}
	...
	for (auto connection: gConnections)
	{
		if (connection.first == bmdVideoConnectionUnspecified || (ports & connection.first))
		{
			...
			for (const auto& conversion: gInputConversions)
			{
				...
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, bmdSupportedVideoModeDefault, "");
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, bmdSupportedVideoModePsF, "PsF");
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, bmdSupportedVideoModeDualStream3D, "3D");
				print_input_modes_for_setup(deckLinkInput, connection.first, conversion.first, (BMDSupportedVideoModeFlags)(bmdSupportedVideoModePsF | bmdSupportedVideoModeDualStream3D), "PsF3D");
			}
		}
	}
