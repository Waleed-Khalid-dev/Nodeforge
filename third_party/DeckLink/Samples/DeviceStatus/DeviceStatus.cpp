	...
	m_ui(new Ui::DeviceStatusDialog)
{
	...
	connect(m_ui->ethernetConnectorComboBox,  QOverload<int>::of(&QComboBox::currentIndexChanged), this, &DeviceStatus::ethernetConnectorChanged);

	m_statusDataTableModel = new DeckLinkStatusDataTableModel(this);
	connect(m_statusDataTableModel, &QAbstractTableModel::rowsInserted, this, [this](){ updateRowAndColumnSizes(m_ui->statusDataTableView); });
	...
	m_ethernetStatusDataTableModel = new DeckLinkEthernetStatusDataTableModel(this);
	connect(m_statusDataTableModel, &QAbstractTableModel::rowsInserted, this, [this](){ updateRowAndColumnSizes(m_ui->ethernetStatusDataTableView); });
	m_ui->ethernetStatusDataTableView->setModel(m_ethernetStatusDataTableModel);
	m_ui->ethernetStatusDataTableView->verticalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
	m_ui->ethernetStatusDataTableView->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
	m_ui->ethernetStatusDataTableView->horizontalHeader()->setStretchLastSection(true);
...
void DeviceStatus::setup()
{
	...
	m_ethernetStatusDataTableModel->reset(m_selectedDeckLink);
}
...
void DeviceStatus::closeEvent(QCloseEvent* event)
{
	if (m_selectedDeckLink)
	{
		...
		if (deckLinkNotification)
		{
			...
			deckLinkNotification->Unsubscribe(bmdStatusChanged, m_ethernetStatusDataTableModel->delegate());
		}
	}
...
{
	int64_t ethernetConnectorCount = 0;
	...
	if (m_selectedDeckLink)
	{
		com_ptr<IDeckLinkProfileManager>		deckLinkProfileManager(IID_IDeckLinkProfileManager, m_selectedDeckLink);
		com_ptr<IDeckLinkNotification>			deckLinkNotification(IID_IDeckLinkNotification, m_selectedDeckLink);
		com_ptr<IDeckLinkProfileAttributes>		deckLinkAttributes(IID_IDeckLinkProfileAttributes, m_selectedDeckLink);
		...
		if (deckLinkNotification)
		{
			...
			deckLinkNotification->Subscribe(bmdStatusChanged, m_ethernetStatusDataTableModel->delegate());
		}
				
		if (deckLinkAttributes)
		{
			if (deckLinkAttributes->GetInt(BMDDeckLinkNumberOfEthernetConnectors, &ethernetConnectorCount) != S_OK)
				ethernetConnectorCount = 0;
		}
	}

	updateDuplexMode(m_selectedDeckLink);
	updateEthernetConnectors(ethernetConnectorCount);

	// Reset status data table with initial status values
	m_statusDataTableModel->reset(m_selectedDeckLink);
	m_ethernetStatusDataTableModel->reset(m_selectedDeckLink);
	
	rebuildTabs(ethernetConnectorCount > 0);
}

void DeviceStatus::ethernetConnectorChanged(int selectedConnectorIndex)
{
	m_ethernetStatusDataTableModel->selectEthernetConnector(selectedConnectorIndex);
	m_ethernetStatusDataTableModel->reset(m_selectedDeckLink);
}

void DeviceStatus::updateRowAndColumnSizes(QTableView* tableView)
{
...
void DeviceStatus::updateEthernetConnectors(int64_t connectorCount)
{
	m_ui->ethernetConnectorComboBox->clear();
	
	for (uint64_t connector = 0; connector < connectorCount; ++connector)
		m_ui->ethernetConnectorComboBox->addItem(QString::number(connector));
}

void DeviceStatus::rebuildTabs(bool hasEthernet)
{
	m_ui->tabWidget->clear();
	
	m_ui->tabWidget->addTab(m_ui->statusTab, "DeckLink Status");
	if (hasEthernet)
		m_ui->tabWidget->addTab(m_ui->ethernetTab, "Ethernet Status");
}
