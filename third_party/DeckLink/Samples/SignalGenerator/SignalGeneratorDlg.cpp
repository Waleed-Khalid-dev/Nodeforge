...
void CSignalGeneratorDlg::RefreshPixelFormatMenu(void)
{
	...
	int currentSelection = m_pixelFormatCombo.GetCurSel();
	BMDPixelFormat currentPixelFormat = (currentSelection != CB_ERR) ? (BMDPixelFormat)m_pixelFormatCombo.GetItemData(currentSelection) : bmdFormatUnspecified;
	...
	for (auto& pixelFormat : kPixelFormats)
	{
		...
		if (currentPixelFormat == pixelFormat.first)
			m_pixelFormatCombo.SetCurSel(newIndex);
	}

	// If current selection is not supported, default to first item
	if (m_pixelFormatCombo.GetCurSel() == CB_ERR)
