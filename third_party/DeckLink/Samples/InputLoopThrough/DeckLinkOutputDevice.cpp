	...
	if (completedFrame)
	{
		...
		if (m_deckLinkOutput->GetFrameCompletionReferenceTimestamp(completedFrame, ReferenceTime::kTimescale, &frameCompletionTimestamp) == S_OK)
		{
			...
			for (auto iter = m_scheduledFramesList.rbegin(); iter != m_scheduledFramesList.rend(); iter++)
			{
				...
				if (loopThroughVideoFrame->getVideoFrame() == com_ptr<IDeckLinkVideoFrame>(completedFrame))
				{
...
bool DeckLinkOutputDevice::hasEthernetConnection()
{
	...
	int64_t outputConnections;
	if (deckLinkAttributes->GetInt(BMDDeckLinkVideoOutputConnections, &outputConnections) != S_OK)
		return false;

	return (outputConnections & (bmdVideoConnectionEthernet | bmdVideoConnectionOpticalEthernet)) != 0;
}
...
void DeckLinkOutputDevice::scheduleVideoFramesThread()
{
	while (true)
	{
		...
		if (m_outputVideoFrameQueue.waitForSample(outputFrame))
		{
			...
			if (m_deckLinkOutput->ScheduleVideoFrame(outputFrame->getVideoFrame().get(), outputFrame->getVideoStreamTime(), m_frameDuration, m_frameTimescale) != S_OK)
			{
